defmodule FuzzyCatalog.Catalog.Providers.GoogleBooksProvider do
  ...
  def lookup_by_title(title) when is_binary(title) do
    case String.trim(title) do
      ...
      clean_title ->
        ...
        url = "#{@google_books_url}?q=intitle:#{encoded_title}&fields=items(volumeInfo(title,subtitle,authors,publisher,publishedDate,pageCount,categories,description,seriesInfo,industryIdentifiers,imageLinks))&maxResults=10"
  ...
  defp lookup_google_books_full(isbn) do
    url = "#{@google_books_url}?q=isbn:#{isbn}&fields=items(volumeInfo(title,subtitle,authors,publisher,publishedDate,pageCount,categories,description,seriesInfo,industryIdentifiers,imageLinks))"
  ...
  defp extract_google_series(%{"volumeSeries" => [%{"seriesId" => series_name} | _]}) when is_binary(series_name) do
    series_name
  end
